...
function isURLValid(options) {
    if (options._all.url) {
        try {
            assert(validator.isURL(options._all.url, {
                protocols: ["http", "https"],
                require_host: true,
                require_valid_protocol: true,
                require_protocols: true
            }), options._all.url + " is invalid.");
        } catch (err) {
            console.error(err.message);
            return 1;
        }
...
function isNestingValid(options) {
    if (options._all.nesting !== undefined) {
        try {
            assert(validator.isInt(options._all.nesting.toString(), {
                gt: 0
...
function isConcurrencyValid(options) {
    if (options._all.concurrency !== undefined) {
        try {
            assert(validator.isInt(options._all.concurrency + "", {
                gt: 0
            }), "Concurency must be greater than 0");
        } catch (err) {
            console.error(err.message);
            return 1;
        }
